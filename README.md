...
- **v1.8.1** (2026-08-04) — Fixed two SKILL.md descriptions that exceeded Claude Code's per-skill listing cap (`skillListingMaxDescChars`, 1536) and were therefore being truncated mid-word in every turn's context: `cross-worktree-spec-handoff-via-checkout-paths` (2,146 → 1,485 chars; 610 chars, including the entire v1.3.0 cwd-relative-pathspec variant, had been invisible to the model since v1.6.0) and `pre-dispatch-schema-probe` (1,539 → 1,424). Every distinct concept and trigger phrase was preserved; only version annotations, duplicated prose, and detail already covered by a sibling skill were cut. Added the cap as a CI + pre-push gate (`scripts/check_skill_descriptions.py`, vendored from [context-police](https://github.com/wan-huiyan/context-police)). No skills added or removed. Total: 94 skills.
